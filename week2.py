...
def distance(x0, x1):
    ...
    distance = np.sqrt(distance)# <-- EDIT THIS to compute the distance between x0 and x1
    return distance
...
def angle(x0, x1):
    ...
    lx0 = np.sqrt(x0.T @ x0)
    lx1 = np.sqrt(x1.T @ x1)
    angle = (x0.T @ x1) / (lx0 * lx1)
    angle = np.arccos(angle)# <-- EDIT THIS to compute angle between x0 and x1
    return angle
